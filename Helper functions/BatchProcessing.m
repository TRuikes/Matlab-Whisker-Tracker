...
for i = 1:length(vid_files)
    disp_names{i} = fullfile(vid_files(i).folder, vid_files(i).name);
    disp_names{i} = disp_names{i}(length(PathName)+1:end-4);
end

for i = 1:size(track_files,1)
    track_files(i).vid = [track_files(i).name(1:end-12) extension];
end
...
for i = 1:length(vid_files)
    ...
    for j = 1:size(track_files,1)
        t= fullfile(track_files(j).folder, track_files(j).vid);
        v = fullfile(vid_files(i).folder, vid_files(i).name);
